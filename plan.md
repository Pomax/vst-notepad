...
**The open problem: DAWs list the plugin as an instrument as well as an effect.**
Three attempted fixes have not resolved it.

Facts established, so they need not be re-derived:

- The current build declares, through all three factory versions: two classes
  (`Audio Module Class` + `Component Controller Class`), subcategory `Fx` on the
  effect class and empty on the controller, one audio input bus, one audio
  output bus, zero event buses, and it refuses any configuration with no audio
  input. Verified by reading it back out of the built binary.
- The struct layouts used to verify that are trustworthy: the same code reads
  the same fields correctly out of Surge XT Effects and iZotope RX 9.
- Ruled out: the subcategory string, a missing `IPluginFactory3`, DAW-side
  caching, and the enum values behind media type / bus direction.

**Untested:** the current build has never been loaded in a DAW. As of the end of
this session `C:\Program Files\Common Files\VST3\` contains no Notepad at all,
so whatever the DAWs are showing comes from a scan of an older binary. The
two-class split in particular has not been tried.

Next step: install `dist/Notepad.vst3` (5,801,984 bytes), rescan one DAW, and if
it is still wrong, say which DAW. If the two-class split turns out not to be the
cause, revert it — it is machinery a notepad should not need.
...
2. **It was a single-component effect.** One class implemented both
   `IComponent` and `IEditController`, with `getControllerClassId` returning
   `kNotImplemented`. Legal VST3, but no shipping plugin does it, and the SDK's
   `again` example declares two classes: an audio-effect class carrying `Fx`,
   and a component-controller class with an empty subcategory. The plugin now
   does the same.

The document still lives in one place: the processor owns it, and hands the
controller a reference to the same `Editor` when the host connects the pair
over `IConnectionPoint`. That is sound because the plugin does not declare
`kDistributable`, which is what would let a host split the halves across
processes.
